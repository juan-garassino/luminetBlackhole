...
def test_varying_mass(mass):
    ...
    for incl in (0, 45, 90, 135):
        ...
        bh.calc_isoradials(
            [10, 20, 30], [10, 20, 30]
        )  # calculate some isoradials, should be quick enough
        for radius, ghost_and_direct in bh.isoradials.items():
            ...
            for isoradial in (ghost_and_direct[1], ghost_and_direct[0]):
                assert not any(
                    np.isnan(isoradial.radii_b)
                ), "Isoradials contain nan values"
                assert not any(
                    np.isnan(isoradial.angles)
                ), "Isoradials contain nan values"
        bh.calc_isoredshifts([-0.1, 0, 0.2])  # test some isoredshifts, somewhat slower
    return None
