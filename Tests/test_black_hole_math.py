...
@pytest.mark.parametrize(
    "radius", [np.random.randint(6.01, 60.0) for _ in range(N_radii)]
)
@pytest.mark.parametrize(
    "order", [0.0, 1.0, 2.0]
...
class TestParametrized:
    def test_calc_periastron(self, mass, inclination, angle, radius, order):
        ...
        """
        p = calc_periastron(
            _r=radius * mass, incl=inclination, _alpha=angle, bh_mass=mass, n=order
        )
        return None

    def test_get_b_from_periastron(self, mass, inclination, angle, radius, order):
        b = calc_impact_parameter(
            _r=radius, incl=inclination, _alpha=angle, n=order, bh_mass=mass
        )
