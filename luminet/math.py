...
def calc_q(periastron: float, bh_mass: float, tol=1e-3) -> float:
    ...
    q = np.sqrt((periastron - 2.0 * bh_mass) * (periastron + 6.0 * bh_mass))
    # Q is complex if P < 2M = r_s
    return q


def calc_b_from_periastron(
    periastron: float, bh_mass: float, tol: float = 1e-5
) -> float:
    """
    ...
    return np.sqrt(
        periastron**3 / (periastron - 2.0 * bh_mass)
...
def k2(periastron: float, bh_mass: float, tol: float = 1e-6):
    ...
    #     return 0.
    # WARNING: Paper has an error here. There should be brackets around the numerator.
    return (q - periastron + 6 * bh_mass) / (
        2 * q
    )  # the modulus of the ellipitic integral
...
def filter_periastrons(periastron: [], bh_mass: float, tol: float = 10e-3) -> []:
    ...
    """
    return [e for e in periastron if abs(e - 2.0 * bh_mass) > tol]
...
def eq13(
    ...
    sn2 = sn * sn
    term1 = -(q - periastron + 2.0 * bh_mass) / (4.0 * bh_mass * periastron)
    term2 = ((q - periastron + 6.0 * bh_mass) / (4.0 * bh_mass * periastron)) * sn2

    return 1.0 - ir_radius * (term1 + term2)  # solve this for zero
...
def midpoint_method(func, args: Dict, __x, __y, __ind):
    ...
    inbetween_x = np.mean(
        x_
    )  # new periastron value, closer to solution yielding 0 for ea13
    ...
    return (
        ...
        new_y,
        new_ind,
    )  # return x and y refined in relevant regions, as well as new index of sign change


def improve_solutions_midpoint(
    func, args, x, y, index_of_sign_change, iterations
) -> float:
    """
    ...
    new_y = y
    new_ind = (
        index_of_sign_change_  # location in X and Y where eq13(P=X[ind]) equals Y=0
    )
    for iteration in range(iterations):
        new_x, new_y, new_ind = midpoint_method(
            func=func, args=args, __x=new_x, __y=new_y, __ind=new_ind
        )
...
def calc_periastron(
    ...
    periastron_range = list(np.linspace(min_periastron, 2.0 * _r, initial_guesses))
    y_ = [
        eq13(P_value, _r, _alpha, bh_mass, incl, n) for P_value in periastron_range
    ]  # values of eq13
    ind = np.where(np.diff(np.sign(y_)))[0]  # only one solution should exist
    periastron_solution = (
        periastron_range[ind[0]] if len(ind) else None
    )  # initial guesses for P

    if (periastron_solution is not None) and (
        not np.isnan(periastron_solution)
    ):  # elliptic integral found a periastron solving equation 13
        args_eq13 = {
            "ir_radius": _r,
            "ir_angle": _alpha,
            "bh_mass": bh_mass,
        ...
        periastron_solution = improve_solutions_midpoint(
            func=eq13,
            args=args_eq13,
            x=periastron_range,
            y=y_,
            index_of_sign_change=ind[0],
...
def calc_impact_parameter(
    ...
    if periastron_solution is None or periastron_solution <= 2.0 * bh_mass:
    ...
    elif periastron_solution > 2.0 * bh_mass:
        ...
        # plt.show()
        raise ValueError(
            f"No solution was found for the periastron at (r, a) = ({_r}, {_alpha}) and incl={incl}"
        )
...
def redshift_factor(radius, angle, incl, bh_mass, b_):
    ...
    # gtt = - (1 - (2. * M) / radius)
    z_factor = (
        1.0 + np.sqrt(bh_mass / (radius**3)) * b_ * np.sin(incl) * np.sin(angle)
